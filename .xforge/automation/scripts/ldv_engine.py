...
def decompose_tasks(analysis: dict) -> dict:
    ...
    for i in range(count):
        ...
        nodes.append({
            "id": tid,
            "title": f"Step {i+1}: {analysis['originalRequest'][:40]}...",
    ...
    return {
        "totalTasks": count,
        "dag": {"nodes": nodes, "edges": edges, "entryPoints": ["T-001"] if count > 0 else [], "exitPoints": [f"T-{count:03d}"] if count > 0 else []},
...
def run_loop(request: str) -> dict:
    ...
    return {
        ...
        "summary": {"totalTasks": decomp["totalTasks"], "completed": completed, "failed": 0, "skipped": 0}
    }

def cli():
    p = argparse.ArgumentParser(description="LDV Engine - Loop de Decomposicao com Validacao")
    ...
    if args.cmd == "analyze":
        ...
        su = s.get("summary", {})
        print(f"Tasks: {su.get('completed',0)}/{su.get('totalTasks',0)} completed, {su.get('failed',0)} failed")
