    ...
    _cmdAnalyze() {
        ...
        try {
            ...
            if (files.includes('package.json'))
                stackHints.push('Node.js / TypeScript');
            ...
            if (files.includes('pom.xml') || files.includes('build.gradle'))
                stackHints.push('Java / Kotlin');
            if (files.includes('.sln') || files.includes('*.csproj'))
                stackHints.push('.NET');
            if (files.includes('index.html'))
                stackHints.push('HTML / Frontend');
            if ((fs.readdirSync(root).filter(f => f.endsWith('.ts') || f.endsWith('.tsx')).length) > 0)
                stackHints.push('TypeScript files detected');
        }
        catch { /* ignore */ }
        return [
            '## Analise de Projeto',
