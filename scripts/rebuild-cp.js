// Rebuild Script — escreve novo ChatViewProvider.ts limpo via fs.writeFileSync
const fs = require('fs');
...
export class ChatViewProvider implements vscode.WebviewViewProvider {
    ...
    private _getSharedStyles(): string {
        return [
            ...
            'body { font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif; font-size:13px; background:var(--vscode-sideBar-background,#1e1e1e); color:var(--vscode-foreground,#ccc); height:100vh; display:flex; flex-direction:column; }',
    ...
    private async _callProvider(userText: string) {
        ...
        try {
            ...
            this.postMessage({ type: 'streamEnd', id: assistant assistantMessage.content });
        } catch (err) {
...
export class SettingsViewProvider implements vscode.WebviewViewProvider {
    ...
    resolveWebviewView(webviewView: vscode.WebviewView) {
        ...
        const cards = saved.filter((p: StoredProvider) => p.enabled).map((p: StoredProvider) => {
            ...
            return '<div class="card' + (isActive ? ' card-active' : '') + '" style="padding:10px;border:1px solid var(--vscode-widget-border,#3c3c3c);border-radius:6px;margin-bottom:8px;cursor:pointer;"><div style="display:flex;justify-content:space-between;"><span style="font-weight:600;font-size:.85rem;">' + (ACTIVE ? 'ok ' : '') + p.label + '</span><span style="font-size:.7rem;color:#888;">' + keyMask + '</span></div><div style="font-size:.75rem;color:#888;margin-top:4px;">' + (isActive ? ('model: ' + sel.model) : 'clique p/ ativar') + '</div></div>';
        }).join('');
...
const finalContent = HEADER + OTHER_CLASSES;
const targetPath = path.join(__dirname, '..', 'src', 'views', 'ChatViewProvider.ts');
fs.writeFileSync(targetPath, finalContent, 'utf-8');
console.log('OK — wrote ' + finalContent.length + ' bytes to ChatViewProvider.ts');
